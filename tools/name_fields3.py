    ...
    for (sr, dr), _ in Counter((p[0], p[2]) for p in pairs).most_common(3):
        ...
        for msg in unique:
            ...
            if len(inter) < 6 or len(inter) / len(srcs) < 0.85:
                continue
            # type must agree with how the value was moved. A block copy ('b')
            # carries no type information, so it constrains nothing.
            bad = False
            for o in inter:
                t = lay[o][1]
...
p = os.path.join(OUT, 'named_fields_wide.json')
