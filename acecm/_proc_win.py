"""Windows process helpers — the NT implementation behind `winproc`.

Extracted verbatim from winproc.py when Linux support landed. Nothing here
changed; `winproc` picks this module on Windows and `_proc_posix` elsewhere.
Never shell out to powershell.exe: Toolhelp / psapi stay in-process.
"""
import ctypes
import struct
...
def ppid(pid):
    """Parent pid, or 0 if unknown. From the Toolhelp snapshot, no powershell."""
...
def cmdline(pid):
    """Full command line of `pid`, or "".

    ⚠ The one place this module knowingly spawns powershell. There is no
    Toolhelp field for the command line, and the WMI/NtQueryInformationProcess
    routes are both worse. It runs once, only when reclaiming an orphaned
    port, not on a hot path.
    """
    import subprocess
    try:
        r = subprocess.run(
